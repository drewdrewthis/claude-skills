...
## Detect default branch and timezone

Before gathering data:

1. Detect the repo's default branch name:
   `gh repo view --json defaultBranchRef -q .defaultBranchRef.name`
   If this fails, fall back to `main`. Use the detected name wherever the instructions say `origin/<default>` below.

2. Detect the machine's local timezone:
   ```bash
   LOCAL_TZ=$(readlink /etc/localtime 2>/dev/null | sed 's|.*/zoneinfo/||' || echo "UTC")
   ```
   Use `TZ=$LOCAL_TZ` wherever the instructions say `TZ=<local>` below. Report all times in local time.
...
Parse the argument to determine the time window. Default to 7 days if no argument given. Use `--since="N days ago"`, `--since="N hours ago"`, or `--since="N weeks ago"` (for `w` units) for git log queries. All times should be reported in **local time** (use `TZ=<local>` when converting timestamps).
...
#    Use TZ=<local> for local time conversion
TZ=<local> git log origin/<default> --since="<window>" --format="%at|%aN|%ai|%s" | sort -n
...
```bash
# Team streak: all unique commit dates (local time)
TZ=<local> git log origin/<default> --format="%ad" --date=format:"%Y-%m-%d" | sort -u

# Personal streak: only the current user's commits
TZ=<local> git log origin/<default> --author="<user_name>" --format="%ad" --date=format:"%Y-%m-%d" | sort -u
```
...
```json
{
  ...
  "authors": {
    "Alice": { "commits": 32, "insertions": 2400, "deletions": 300, "test_ratio": 0.41, "top_area": "app/" },
    "Bob": { "commits": 12, "insertions": 800, "deletions": 150, "test_ratio": 0.35, "top_area": "services/" }
  },
  "streak_days": 47,
...
#### [Name]
- **What they shipped**: 2-3 sentences on their contributions
- **Praise**: 1-2 specific things they did well, anchored in actual commits
...
### Top 3 Team Wins
Identify the 3 highest-impact things shipped. For each: what, who, why it matters.
...
2. Compute metrics for the immediately prior same-length window using both `--since` and `--until` to avoid overlap
...
- Convert all timestamps to local time for display (use `TZ=<local>` detected in setup)
