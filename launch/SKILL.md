...
1. Write a launcher script. Use `claude --dangerously-skip-permissions` for unattended sessions. Send `/rename` as the initial prompt, then send the work instruction as a separate message after Claude starts:
```bash
...
3. After Claude starts, send the work instruction as a separate message. Use `/orchestrate` if available (from the orchestrate plugin), otherwise tell Claude to read the issue and implement it:
```bash
sleep 10
# If /orchestrate is available (orchestrate plugin installed):
#   tmux send-keys -t "$TMUX_SESSION" "/orchestrate #<N>" Enter
# Otherwise, send a direct instruction:
tmux send-keys -t "$TMUX_SESSION" "Read issue #<N> with gh issue view <N>. Implement the requirements, write tests, and open a PR when done." Enter
sleep 3
